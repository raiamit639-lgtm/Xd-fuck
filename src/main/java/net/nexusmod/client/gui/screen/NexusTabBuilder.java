...
public final class NexusTabBuilder {
    ...
    private static final class Layout {
        ...
        final class CardBuilder {
            ...
            void slider(Text label, int min, int max, int initial, java.util.function.Function<Integer, Text> fmt,
                        java.util.function.Consumer<Integer> onChange) {
                if (!matches(label)) return;
                NexusLabel lbl = new NexusLabel(cardX + 12, rowY, 110, 18, label);
                NexusSlider sld = new NexusSlider(cardX + 126, rowY - 2, colWidth - 138, 18, min, max, initial, fmt, onChange);
                addRow(lbl, sld);
            }
